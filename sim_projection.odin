...
do_gradient :: proc(pressure_texture, velocity_x_texture, velocity_y_texture, velocity_z_texture: u32, size: [3]i32) {
    ...
    gl.BindImageTexture(0, velocity_x_texture, 0, gl.TRUE, 0, gl.WRITE_ONLY, TEXTURE_PRECISION);
    gl.BindImageTexture(1, velocity_y_texture, 0, gl.TRUE, 0, gl.WRITE_ONLY, TEXTURE_PRECISION);
    gl.BindImageTexture(2, velocity_z_texture, 0, gl.TRUE, 0, gl.WRITE_ONLY, TEXTURE_PRECISION);
...
do_zero_pressure :: proc(pressure_ping_texture: u32, size: [3]i32){
    ...
    gl.BindImageTexture(0, pressure_ping_texture, 0, gl.TRUE, 0, gl.WRITE_ONLY, TEXTURE_PRECISION);
...
do_sor :: proc(ping_texture, pong_texture: ^u32, divergence_texture: u32, omega: f32, size: [3]i32, iterations: int) {
    ...
    for i in 0..<2*iterations {
        gl.BindTextureUnit(0, ping_texture^);
        gl.BindImageTexture(0, ping_texture^, 0, gl.TRUE, 0, gl.WRITE_ONLY, TEXTURE_PRECISION);
...
do_prolongate :: proc(coarse_pressure_texture, fine_pressure_texture: u32, coarse_size, fine_size: [3]i32)  {
    ...
    gl.BindImageTexture(0, fine_pressure_texture, 0, gl.TRUE, 0, gl.WRITE_ONLY, TEXTURE_PRECISION);
    gl.MemoryBarrier(gl.TEXTURE_FETCH_BARRIER_BIT)
...
vcycle :: proc(current_level, max_level: Resolution) {
    ...
    if ctx.smooth_weight >= 1.0 {
        do_sor(&ctx.pressure_ping_textures[current_level], &ctx.pressure_pong_textures[current_level], ctx.rhs_textures[current_level], ctx.smooth_weight, ctx.sizes[current_level], ctx.pre_smooths0 if current_level == .X1 else ctx.pre_smooths1)
    } else {
        do_jacobi(&ctx.pressure_ping_textures[current_level], &ctx.pressure_pong_textures[current_level], ctx.rhs_textures[current_level], ctx.smooth_weight, ctx.sizes[current_level], ctx.pre_smooths0 if current_level == .X1 else ctx.pre_smooths1)
    }
    ...
    if ctx.smooth_weight >= 1.0 {
        do_sor(&ctx.pressure_ping_textures[current_level], &ctx.pressure_pong_textures[current_level], ctx.rhs_textures[current_level], ctx.smooth_weight, ctx.sizes[current_level], ctx.post_smooths0 if current_level == .X1 else ctx.post_smooths1)
    } else {
        do_jacobi(&ctx.pressure_ping_textures[current_level], &ctx.pressure_pong_textures[current_level], ctx.rhs_textures[current_level], ctx.smooth_weight, ctx.sizes[current_level], ctx.post_smooths0 if current_level == .X1 else ctx.post_smooths1)
    }
}