...
init_from_ttf :: proc(ttf_name, identifier: string, oversample: [2]int, sizes: []int, codepoints: []rune, width := 2048) -> (Font, bool) {
	...
	for _, i in sizes {
		pack_ranges[i] = Pack_Range{f32(sizes[i]), 0, &codepoints[0], i32(len(codepoints)), &glyph_metrics[i*len(codepoints)+0],  0, 0};
	}

	// do the actual packing of the glyphs
	pc: stbtt.pack_context
	ret := stbtt.PackBegin(&pc, &bitmap_raster[0], i32(width), i32(height), 0, 1, nil);
	stbtt.PackSetOversampling(&pc, u32(oversample[0]), u32(oversample[1])); 
	stbtt.PackFontRanges(&pc, &ttf_data[0], 0, &pack_ranges[0], i32(len(pack_ranges)));
	stbtt.PackEnd(&pc);
	...
	info: Font_Info;
	stbtt.InitFont(&info, &ttf_data[0], stbtt.GetFontOffsetForIndex(&ttf_data[0], 0));
	for _, i in sizes {
		using size_metric := &size_metrics[i];

		scale := stbtt.ScaleForPixelHeight(&info, f32(sizes[i]));
		a, d, l: i32
		stbtt.GetFontVMetrics(&info, &a, &d, &l);
