...
init_from_ttf_gl :: proc(ttf_name, identifier: string, use_subpixels: bool, sizes: []int, codepoints: []rune) -> (Font, bool) {
    ...
    gl.Uniform2f(1, f32(font.width), f32(font.height)); 
    gl.Uniform1i(3, i32(use_subpixels));
    gl.Uniform1i(4, 0);
...
set_state :: proc(in_vao: u32 = 0) {
    ...
    gl.GetIntegerv(gl.CURRENT_PROGRAM, &last_program);
	gl.UseProgram(program);
    gl.Uniform2f(0, f32(dims[2]), f32(dims[3]));
    gl.UseProgram(u32(last_program));
}
...
draw_instances :: proc(instances: []Glyph_Instance, offset: [2]f32) {
    ...
    gl.GetIntegerv(gl.CURRENT_PROGRAM, &last_program);
    gl.UseProgram(program);
	gl.Uniform2f(2, offset.x, offset.y); 
...
layout(location = 0) uniform vec2 window_resolution;
layout(location = 1) uniform vec2 bitmap_resolution;
layout(location = 2) uniform vec2 string_offset = vec2(0.0);
...
layout(location = 1) uniform vec2 bitmap_resolution;
layout(location = 3) uniform bool use_subpixels;
layout(location = 4) uniform sampler2D sampler_bitmap;
