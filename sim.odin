...
do_sim_step :: proc() {
    ...
    gl.MemoryBarrier(gl.TEXTURE_FETCH_BARRIER_BIT)
    {
        ...
        gl.BindImageTexture(3, ctx.final_divergence_textures[.X1], 0, gl.TRUE, 0, gl.WRITE_ONLY, TEXTURE_PRECISION);
    ...
    gl.MemoryBarrier(gl.TEXTURE_FETCH_BARRIER_BIT)
    {
        ...
        gl.BindImageTexture(0, ctx.velocity_z_textures[.X1], 0, gl.TRUE, 0, gl.WRITE_ONLY, TEXTURE_PRECISION);
...
do_sim_reset :: proc() {
    fmt.println("Resetting Data")
    {
        ...
        gl.DispatchCompute(expand_values(linalg.to_u32(ctx.sizes[.X1] / 8)))
        gl.BindImageTexture(0, ctx.velocity_x_textures[.X1], 0, gl.TRUE, 0, gl.WRITE_ONLY, TEXTURE_PRECISION);
        gl.DispatchCompute(expand_values(linalg.to_u32(ctx.sizes[.X1] / 8)))
        gl.BindImageTexture(0, ctx.velocity_y_textures[.X1], 0, gl.TRUE, 0, gl.WRITE_ONLY, TEXTURE_PRECISION);
        gl.DispatchCompute(expand_values(linalg.to_u32(ctx.sizes[.X1] / 8)))
        gl.BindImageTexture(0, ctx.velocity_z_textures[.X1], 0, gl.TRUE, 0, gl.WRITE_ONLY, TEXTURE_PRECISION);
        gl.DispatchCompute(expand_values(linalg.to_u32(ctx.sizes[.X1] / 8)))
    }
