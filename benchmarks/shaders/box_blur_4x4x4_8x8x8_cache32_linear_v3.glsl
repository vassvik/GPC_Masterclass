...
void cache_boundary_x(uvec3 wid, uvec3 lid) {
    uvec3 gid = 8*wid + lid;
...
void cache_boundary_y(uvec3 wid, uvec3 lid) {
    uvec3 gid = 8*wid + lid;
...
void cache_boundary_z(uvec3 wid, uvec3 lid) {
    uvec3 gid = 8*wid + lid;
...
void cache_boundary_xy(uvec3 wid, uvec3 lid) {
    uvec3 gid = 8*wid + lid;
    
    if (any(greaterThanEqual(gid.xy, u_size.xy))) {
        // two-sided out of bounds check
        s_x[1+lid.z][1+lid.y][1+lid.x] = 0.0;
        return;
    } 
...
void cache_boundary_xz(uvec3 wid, uvec3 lid) {
    uvec3 gid = 8*wid + lid;
    
    if (any(greaterThanEqual(gid.xz, u_size.xz))) {
        // two-sided out of bounds check
        s_x[1+lid.z][1+lid.y][1+lid.x] = 0.0;
        return;
    } 
...
void cache_boundary_yz(uvec3 wid, uvec3 lid) {
    uvec3 gid = 8*wid + lid;
    
    if (any(greaterThanEqual(gid.yz, u_size.yz))) {
        // two-sided out of bounds check
        s_x[1+lid.z][1+lid.y][1+lid.x] = 0.0;
        return;
    } 
...
void main() {
...

    {
        // Faces
        uvec3 idx = uvec3(
            bfe(tid, 3, 3), // idx.x:  [0, 8)
            bfe(tid, 0, 3), // idx.y:  [0, 8)
            0
        );
        cache_boundary_x(wid, idx.xyz + uvec3( 0,  0, -1)); // Z-faces (Blue)
        cache_boundary_x(wid, idx.xyz + uvec3( 0,  0, +8)); // Z-faces (Blue)
        cache_boundary_y(wid, idx.xzy + uvec3( 0, -1,  0)); // Y-faces (Green)
        cache_boundary_y(wid, idx.xzy + uvec3( 0, +8,  0)); // Y-faces (Green)
        cache_boundary_z(wid, idx.zxy + uvec3(-1,  0,  0)); // X-faces (Red)
        cache_boundary_z(wid, idx.zxy + uvec3(+8,  0,  0)); // X-faces (Red)
    } 
    ...
    {   
        ...
        uvec3 idx = uvec3(
                   bfe(tid, 0, 3), // idx.x:  [0, 8)
            -1 + 9*bfe(tid, 4, 1), // idx.y:  {-1, +8}
            -1 + 9*bfe(tid, 3, 1)  // idx.z:  {-1, +8}
        );
        if (tid < 32) cache_boundary_yz(wid, idx.xyz); // YZ-edges (Cyan)
        else          cache_boundary_xz(wid, idx.yxz); // XZ-edges (Magenta)
        if (tid < 32) cache_boundary_xy(wid, idx.yzx); // XY-edges (Yellow)
        else if (tid < 32 + 8) {
            // Corners
            uvec3 idx = uvec3(
                -1 + 9*bfe(tid, 2, 1), // idx.x: {-1, +8}
                -1 + 9*bfe(tid, 1, 1), // idx.y: {-1, +8}
                -1 + 9*bfe(tid, 0, 1)  // idx.z: {-1, +8}
            );
